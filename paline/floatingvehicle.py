...
SPACE_BUFFER = 150     # Il bus si deve spostare di almeno (150) m perché sia considerato un nuovo campione 
#ALGO = 'exp'
ALGO = 'avg'
...
class FVPath(object):
			...
			for new in self.raw[id]:
				if old is not None:
					...
					"""
						...
						if dt <= 0:
							logging.error("Dato duplicato: (%s, %s)" % (str(old), str(new)))
						else:
							logging.error("Spazio negativo (possibile nuovo ciclo): (%s, %s)" % (str(old), str(new)))
					"""
				old = new
			if old is not None:
				self.raw[id] = [old]
	...
	def compute_speed(self, time, place_start, place_stop=None):
		...
		for el in self.speed:
			...
			wp = (MAX_TIME - dt - dp / EQUIV_SPEED) / float(MAX_TIME)
			if wp > 0:
				if ALGO == 'exp':
